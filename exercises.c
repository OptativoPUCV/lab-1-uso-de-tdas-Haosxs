...
int parentesisBalanceados(char *cadena) {
   ...
   for (int i = 0; cadena[i] != '\0'; i++) {
       ...
       {
         if (top(auxiliar) == NULL) return 0;
           if ((cadena[i] == ')' && pop(auxiliar) != '(') || (cadena[i] == '}' && pop(auxiliar) != '{') || (cadena[i] == ']' && pop(auxiliar) != '[')) return 0;
       }
   }
   return top(auxiliar) == NULL;
}
